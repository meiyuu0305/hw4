...
void buildHeap(vector <int> &ptr, int size, int hole, int &count){
    int root = hole;
    while((2*root+2 < size)){
    int left = 2*root+1;
    int right = left+1;
    int change_index = root;
    if(ptr[left] > ptr[root] && ptr[left] > ptr[right]){
        change_index = left;
    }
    else if(ptr[right] > ptr[root] && ptr[right] > ptr[left]){
        change_index = right;
    }
    count++; // comparing elements
    if(change_index != root){
        int temp = ptr[root];
        ptr[root] = ptr[change_index];
        ptr[change_index] = temp;
        root = change_index;
        count++; //swap array element
    }
    root = left;
    }
};

int heap_sort(vector<int> & ptr, int size){
    int count = 0; 
    buildHeap(ptr,size,0, count);
    for(int k = size-1; k >= 0; k--){
        int temp = ptr[0];
        ptr[0] = ptr[k];
        ptr[k] = temp;
        buildHeap(ptr,k,0, count);
    }
    return count;
};
int main() {
...
//     { 
    ...
    comp_ran = heap_sort(test_values, VECTOR_SIZE); 
    comp_asc = heap_sort(asc_test_values, VECTOR_SIZE); 
    comp_desc = heap_sort(desc_test_values, VECTOR_SIZE); 
    //}
